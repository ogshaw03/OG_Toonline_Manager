# -*- coding: utf-8 -*-
"""
OG Toonline Manager (English UI)
================================
Toon outline tool for Maya, built with standard real geometry (inverted hull:
extrude -> reverse normals -> backface culling). Outlines follow the source
mesh deformation/transform, and thickness/color/visibility can be managed and
animated per line.

This is the English-UI build of OG_Toonline_Manager. Functionality is identical
to the Japanese version and both operate on the same scene nodes.

Usage:
    Paste into the Script Editor and run, or
...
class ToonOutlineUI(QtWidgets.QDialog):
    ...
    def _slider_spin_row(self, label, smin, smax, sval, dec, rmin, rmax, step, rval,
        ...
        if on_key:
            ...
            bk.setToolTip("Key this value at the current frame")
            bk.clicked.connect(on_key)
            row.addWidget(bk)
        b = QtWidgets.QPushButton("↺"); b.setFixedWidth(26); b.setToolTip("Reset")
    ...
    def _build(self):
        ...
        arow, self.gslider, self.gspin = self._slider_spin_row(
            "Global Mult", 0, 500, 100, 2, 0.0, 5.0, 0.05, 1.0,
            self._on_gslider, self._on_gspin, self._reset_gmult,
            on_key=self._key_gmult, tip="Thickness multiplier for all outlines")
        b_gsel = QtWidgets.QPushButton("Select")
        b_gsel.setFixedWidth(40)
        b_gsel.setToolTip("Select the global controller (shows keys on the timeline)")
        ...
        self.btn_create = QtWidgets.QPushButton("Outline from Selected Mesh")
        self.btn_create.clicked.connect(self.create_outlines)
        crow.addWidget(self.btn_create, 1)
        self.btn_edge = QtWidgets.QPushButton("Line on Selected Edges")
        self.btn_edge.setToolTip("Add a tube-shaped line along the selected polygon edges")
        self.btn_edge.clicked.connect(self.create_edge_line)
        crow.addWidget(self.btn_edge)
        b_grp = QtWidgets.QPushButton("New Group")
        b_grp.clicked.connect(self.new_group)
        crow.addWidget(b_grp)
        b_ref = QtWidgets.QPushButton("Reacquire")
        ...
        crow2.addWidget(QtWidgets.QLabel("Target Group"))
        ...
        self.tree.setHeaderLabels(["Name", "Thickness", "Color"])
        ...
        self.lbl_dd = QtWidgets.QLabel("* Drag & drop a line onto a group to move it. Double-click to rename.")
        ...
        trow, self.slider, self.spin = self._slider_spin_row(
            "Thickness", 0, 2000, 500, 3, 0.0, 2.0, 0.01, 0.5,
        ...
        c2row, self.cslider, self.cspin = self._slider_spin_row(
            "Curvature", 0, 1000, 0, 2, 0.0, 10.0, 0.1, 0.0,
        ...
        c3row, self.cap_slider, self.cap_spin = self._slider_spin_row(
            "Curvature Cap", 100, 1000, 300, 1, 1.0, 10.0, 0.5, 3.0,
            self._on_cap_slider, self._on_cap_spin, self._reset_cap,
            on_key=self._key_cap, tip="Maximum undulation multiplier (cap so corners do not get too thick)")
        lvl.addLayout(c3row)
        c4row, self.cmin_slider, self.cmin_spin = self._slider_spin_row(
            "Curvature Floor", 0, 100, 100, 2, 0.0, 1.0, 0.05, 1.0,
            self._on_cmin_slider, self._on_cmin_spin, self._reset_cmin,
            on_key=self._key_cmin, tip="Lower bound of the thickness multiplier where curvature is low (flat areas). "
                                       "Lower it to make thin areas even thinner")
        lvl.addLayout(c4row)
        ...
        plabel = QtWidgets.QLabel("Thickness Profile")
        plabel.setAlignment(QtCore.Qt.AlignTop)
        prow.addWidget(plabel)
        self.ramp = _RampWidget()
        self.ramp.setToolTip("Thickness variation along the length. Left-click to add/move a point, right-click to delete.")
        ...
        grow, self.grpslider, self.grpspin = self._slider_spin_row(
            "Group Mult", 0, 500, 100, 2, 0.0, 5.0, 0.05, 1.0,
            self._on_grpslider, self._on_grpspin, self._reset_grpmult,
            on_key=self._key_grpmult, tip="Thickness multiplier for the selected group")
        gvl.addLayout(grow)
        ...
        self.lbl_hint = QtWidgets.QLabel("* Values apply to the line/group selected in the list.")
        self.lbl_hint.setStyleSheet("color:#888;")
        lay.addWidget(self.lbl_hint)

        # 選択物のすべての値を初期値に戻す（実行前に確認ダイアログ）
        self.btn_reset_all = QtWidgets.QPushButton("Reset All Selected")
        self.btn_reset_all.setToolTip("Reset all parameters of the selected line(s)/group(s) to defaults (with confirmation)")
        self.btn_reset_all.clicked.connect(self.reset_selected)
        lay.addWidget(self.btn_reset_all)

        # カラー
        clrow = QtWidgets.QHBoxLayout()
        clrow.addWidget(QtWidgets.QLabel("Shared Color"))
        self.btn_color = QtWidgets.QPushButton()
        self.btn_color.setFixedHeight(22)
        self.btn_color.setToolTip("Change the shared color (default black). Affects all lines using the shared color.")
        self.btn_color.clicked.connect(self._pick_color)
        self._refresh_swatch()
        clrow.addWidget(self.btn_color, 1)
        b_icol = QtWidgets.QPushButton("Individual Color")
        b_scol = QtWidgets.QPushButton("To Shared Color")
        ...
        self.chk_hide_handles = QtWidgets.QCheckBox("Hide deformer handles from the Outliner")
        self.chk_hide_handles.setChecked(True)
        self.chk_hide_handles.setToolTip("When OFF, handles appear in the Outliner (do not delete them)")
        self.chk_hide_handles.toggled.connect(self._on_toggle_hide_handles)
        lay.addWidget(self.chk_hide_handles)

        self.chk_lock_select = QtWidgets.QCheckBox("Make lines unselectable in the viewport")
        self.chk_lock_select.setChecked(True)
        self.chk_lock_select.setToolTip("ON: lines cannot be selected in the viewport (display/render stay enabled). "
                                        "When OFF, they can be selected normally")
        self.chk_lock_select.toggled.connect(self._on_toggle_lock_select)
        lay.addWidget(self.chk_lock_select)

        self.lbl_del = QtWidgets.QLabel("* Delete lines/groups with the Delete key")
    ...
    def _add_line_item(self, parent_item, line):
        is_edge = cmds.attributeQuery(EDGE_TAG, node=line, exists=True)
        label = _short(line) + ("  [Edge]" if is_edge else "  [Hull]")
    ...
    def _add_group_item(self, group_node, lines, visible):
        ...
        label = _short(group_node) if group_node else "(Ungrouped)"
    ...
    def _on_item_changed(self, item, column):
        ...
        if node and cmds.objExists(node):
            ...
            try:
                ...
                cmds.warning("Cannot change visibility of {} (connected/locked)".format(_short(node)))
    ...
    def _apply_group_mult(self, val):
        ...
        try:
            ...
            self._set_ctrl_attr(grpctrl, GMULT, val, "Group Mult")
        finally:
            if chunk:
                cmds.undoInfo(closeChunk=True)
    ...
    def _apply_global_mult(self, val):
        ...
        try:
            ...
            self._set_ctrl_attr(gc, GMULT, val, "Global Mult")
        finally:
            if chunk:
                cmds.undoInfo(closeChunk=True)
        self._update_mult_labels()
    ...
    def _set_ctrl_attr(self, ctrl, attr, val, label, as_string=False):
        ...
        if not settable:
            if plug not in self._warned_connected:
                ...
                why = ("source: " + src[0]) if src else "locked"
                cmds.warning("{}'s \"{}\" is connected to another node and cannot be changed from the UI"
                             " ({}). Remove the animation/connection, or key it with K."
    ...
    def _apply_thickness(self, val):
        ...
        try:
            for line in lines:
                ...
                self._set_ctrl_attr(ctrl, CTRL_THICK, val, "Thickness")
        finally:
            if chunk:
                cmds.undoInfo(closeChunk=True)
        self._update_key_colors()
    ...
    def _apply_profile(self):
        ...
        try:
            for line in lines:
                ...
                self._set_ctrl_attr(ctrl, CTRL_PROFILE, s, "Thickness Profile", as_string=True)
    ...
    def _apply_curvature(self):
        ...
        try:
            for line in lines:
                ...
                ctrl = _ensure_line_anim(line, self.spin.value())
                self._set_ctrl_attr(ctrl, CTRL_CURV, influence, "Curvature")
                self._set_ctrl_attr(ctrl, CTRL_CAP, cap, "Curvature Cap")
                self._set_ctrl_attr(ctrl, CTRL_CMIN, cmin, "Curvature Floor")
                _update_curv_weights(line)
        finally:
            if chunk:
                cmds.undoInfo(closeChunk=True)
    ...
    def _key_line_attr(self, attr):
        lines = self._selected_lines()
        if not lines:
            cmds.warning("Select line(s) to key in the list"); return
        cmds.undoInfo(openChunk=True)
        try:
            for line in lines:
                ctrl = _ensure_line_anim(line, self.spin.value())
    ...
    def _key_grpmult(self):
        grp = self._target_group()
        if not grp:
            cmds.warning("Select group(s) to key in the list"); return
    ...
    def _pick_color(self):
        ...
        c = QtWidgets.QColorDialog.getColor(c0, self, "Shared Color")
        if c.isValid():
    ...
    def set_individual_color(self):
        ...
        lines = self._selected_lines()
        if not lines:
            cmds.warning("Select line(s) to recolor in the list"); return
        init = self._line_color(lines[0]) or [0.0, 0.0, 0.0]
        c = QtWidgets.QColorDialog.getColor(QtGui.QColor.fromRgbF(*init), self, "Individual line color")
        if not c.isValid():
            return
    ...
    def reset_to_shared_color(self):
        ...
        lines = self._selected_lines()
        if not lines:
            cmds.warning("Select line(s) to reset to the shared color"); return
    ...
    def create_outlines(self, *args, **kwargs):
        ...
        if not sel:
            cmds.warning("Please select a mesh"); return
        ...
        try:
            for obj in sel:
                ...
                try:
                    ...
                    cmds.warning("Failed to connect deform-follow (created as a static outline)")
    ...
    def create_edge_line(self, *args):
        ...
        if not edges:
            cmds.warning("Please select mesh edges"); return
    ...
    def rename_node(self, node):
        ...
        try:
            ...
            cmds.warning("Rename failed: {}".format(new))
        finally:
            cmds.undoInfo(closeChunk=True)
        self.refresh_tree()
    ...
    def reset_selected(self):
        ...
        if not lines and not groups:
            cmds.warning("Select item(s) to reset in the list"); return
        # 忠告（確認ダイアログ）。Py2/Py3 両対応のため静的メソッドの warning を使う。
        res = QtWidgets.QMessageBox.warning(
            self, "Reset All Selected",
            "Reset all values of the selected line(s)/group(s) (thickness, curvature, curvature cap, curvature floor, "
            "end taper, profile / group multiplier) to defaults.\n"
            "Current adjustments will be lost (you can undo).\n\nProceed?",
    ...
    def delete_selected(self):
        nodes = self._selected_nodes()
        if not nodes:
            cmds.warning("Select item(s) to delete in the list"); return
